...
const App = () => {
  ...
  const publishBlog = async (blogData) => {
    ...
    try {
      ...
      alert(
        "an error occured, possibly a session time out. try to login again  to see if that fixes the issue. if not, contact our customer care on LOL",
      );
      setUserToken(null);
    }
  };
  ...
  if (userToken !== null) {
    return (
      <div style={bodyStyle}>
        ...
        <h2>blogs</h2>
        <p>
          {window.localStorage.getItem("username")} is logged in{" "}
